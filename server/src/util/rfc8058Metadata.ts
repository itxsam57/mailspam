...
function headerValues(headers: string[], name: string): string[] {
  const pattern = new RegExp(`^${name}\\s*:\\s*([\\s\\S]*)$`, "i");
  return headers
    .map((header) => header.match(pattern)?.[1]?.trim() ?? "")
    .filter(Boolean);
}

export interface Rfc8058RawListHeaders {
...
/**
 * Preserves the complete unfolded List-Unsubscribe value(s) from the bounded
 * raw header section. This avoids mailparser's structured representation
 * discarding alternate URIs such as a leading mailto target. Multiple header
 * fields remain usable for manual actions but are never considered an
 * unambiguous RFC 8058 automatic-one-click declaration.
 */
export function extractRfc8058RawListHeaders(raw: string | Buffer): Rfc8058RawListHeaders {
  const section = boundedRawHeaderSection(raw);
  if (!section) {
    return { listUnsubscribe: null, listUnsubscribePost: null, oneClickHeaderSetUnambiguous: false };
  }
  const headers = unfoldHeaders(section);
  const unsubscribe = headerValues(headers, "List-Unsubscribe");
  const post = headerValues(headers, "List-Unsubscribe-Post");
  return {
    listUnsubscribe: unsubscribe.length ? unsubscribe.join(", ") : null,
    listUnsubscribePost: post.length ? post.join(", ") : null,
    oneClickHeaderSetUnambiguous: unsubscribe.length === 1 && post.length === 1,
  };
}
