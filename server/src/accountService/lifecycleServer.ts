...
const FORBIDDEN_FIELDS = new Set([
  "subject", "body", "bodytext", "rawbody", "htmlsignals", "textpreview",
  "providernativeid", "messageid", "senderaddress", "mailboxaddress",
  "recipientaddress", "mailboxaccountkey", "rawurl", "attachmentbody",
  "apppassword", "accesstoken", "refreshtoken", "oauthcode",
]);
...
function authProof(body: unknown): AccountServiceAuthProof {
  ...
  if (Object.keys(record).some((key) => key !== "challengeId" && key !== "signature")
    ...
    || typeof record.signature !== "string") throw new Error("Device authentication proof is invalid.");
...
export function createAccountLifecycleServer(
  ...
  // Scope all parser/privacy behavior to lifecycle paths only. Unmatched
  // registration/family routes pass untouched to the existing account service.
  app.use("/v1/lifecycle", headers, express.json({ limit: "12kb", strict: true }), (req, res, next) => {
    try {
  ...
  app.post("/v1/lifecycle/auth/challenge", limited("challenge", 30, 60_000), (req, res) => {
    try {
      ...
      if (typeof body.operation !== "string" || !LIFECYCLE_OPERATIONS.has(body.operation as AccountServiceOperation)) throw new Error("Account lifecycle operation is invalid.");
      const challenge = accountService.issueChallenge(body.accountId, body.deviceId, body.operation as AccountServiceOperation);
      noStore(res);
      res.json({ challengeId: challenge.challengeId, challenge: challenge.challenge, operation: challenge.operation, expiresAt: challenge.expiresAt });
    } catch (error) {
  ...
  app.post("/v1/lifecycle/account/export", ...authenticated("account:export", (_req, _res, context) => lifecycle.exportAccount(context.accountId, context.deviceId)));
