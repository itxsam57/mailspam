(() => {
  ...
  const english = Object.freeze({
    ...
    'app.tagline': 'Private local mailbox protection',
    ...
    'connect.fixture': 'Fixture demo mailbox (engineering only)',
    'connect.live': 'Live account (requires provider authorization)',
    'connect.help': 'Live accounts connect directly to the selected provider using provider OAuth or a provider-approved app password.',
    ...
    'scan.stop': 'Stop Scan',
    'background.heading': 'Continuous Protection',
    'background.interval': 'Scheduled fallback interval',
    'background.enable': 'Enable',
    'background.paused': 'Continuous Protection is paused.',
    'background.help': 'One persisted switch authorizes automatic protection for this mailbox. Provider-change processing is used when configured, while the bounded schedule remains the fallback. Manual scans take priority; ad-hoc actions and Analyze Links never run automatically.',
    ...
    'education.controls.heading': 'Actions are scoped',
    'education.controls.body': 'Report Scam saves an account-local campaign rule and moves the current message to Trash after provider confirmation; future campaign matches are auto-moved for this account. Sender/domain blocks do the same for their scope. Global Shield thresholds remain independent. Analyze Links is explicit and never uses mailbox cookies or provider credentials.',
