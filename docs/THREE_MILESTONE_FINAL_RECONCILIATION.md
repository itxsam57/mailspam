...
| 3 — release-ready continuous consumer product | Every repository-buildable pre-native workstream A–V is implemented and literal-head Gate #675 passed Windows/macOS/Ubuntu | **ENGINEERING COMPLETE / FINAL OWNER GATE OPEN** | Final documentation-only freeze gate, then owner live acceptance; native Windows/macOS/Android/iOS shells, store signing/distribution and production infrastructure remain the next/external phase. |
...
| Consumer first-run / zero-confusion dashboard | Eight-step setup requires real account/mailbox presence, completed scan, successful sensitivity save and enabled continuous protection; Family is an explicit optional choice; local Scam Check remains usable before connection | Owner browser acceptance on the frozen release candidate. |

## Exact-head closure evidence

PR #73 fixed the old pull-request CI ambiguity. The Engineering Gate now checks out `github.event.pull_request.head.sha`, asserts `git rev-parse HEAD` equals that immutable SHA, and only then runs the gate.

Gate #675 passed Windows, macOS and Ubuntu plus the combined summary on exact head `59e122d940effa6d5287204bda6741a929401807`. A prior attempt on the same closure sequence correctly failed when documentation invariants became stale; the truth-preserving documentation boundary was restored instead of weakening the test. This reconciliation/status recording commit is documentation-only and must itself receive the same literal-head gate before it becomes the owner handoff SHA.

## Final pre-app decision

A repository release candidate is eligible for owner/external acceptance only when the **unchanged literal PR head SHA** passes the Engineering Gate on Windows, macOS and Ubuntu. The gate includes typecheck, production build, portable-core/provider vectors, Regression Vault, capacity, public-document checks, unit/integration/corpus suites, browser wiring, compiled services/background smokes, portable package verification, signed release lifecycle and dependency audits.

PR #73 remains unmerged until the final documentation-only freeze receives that literal-head gate and the owner completes the generated visible/destructive/recovery acceptance checklist. If acceptance finds a reproducible defect, reproduce it, add regression coverage where technically possible, fix the root cause, rerun the complete exact-head gate, and repeat the failed owner/external check.
