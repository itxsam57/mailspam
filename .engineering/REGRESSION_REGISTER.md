...
| REG-044 | LOCKED | Every explicit guided **Connect Gmail** flow must request a fresh offline refresh token with `access_type=offline` plus `prompt=consent`; a prior Google consent left behind by a failed local setup must not cause a later connection to fail because Google omits `refresh_token`. PKCE S256 and public-client behavior remain mandatory and no `client_secret` may be added to the authorization URL. | `gmailOAuthGoogleCompatibility.test.ts`, strict type/build, Windows/Ubuntu gate |
...
|---|---|---|---|
| GAP-001 | KNOWN-GAP | Guided Gmail OAuth onboarding | Guided desktop Authorization Code + PKCE, verified `sub`, protected refresh-token custody and provider revocation are implemented with automated coverage; controlled real-Google/real-Gmail owner acceptance and production Google consent/scope verification remain before this gap can be closed. |
| GAP-002 | KNOWN-GAP | Guided Outlook OAuth onboarding | Adapter exists; normal browser onboarding is not exposed/live-validated. |
| GAP-003 | KNOWN-GAP | OS-keychain-backed local encryption keys and provider-token custody | Shared fail-closed vault contract and Windows Credential Manager backend exist. Windows iCloud/Yahoo/generic-IMAP app-password sessions use opaque vault references, and guided Gmail refresh-token sessions use stable `sub`-derived opaque vault references on Windows with memory-only compatibility elsewhere. Outlook OAuth secrets and the local policy-encryption key are not yet migrated; macOS Keychain and Linux Secret Service remain unimplemented. |
...
| MAN-016 | MANUAL | Guided Gmail opens Google's consent window, returns through the one-time loopback callback, adds the Gmail account, completes Quick Scan without exposing authorization code/tokens, and a deliberate Disconnect truthfully removes/revokes the account before a later reconnect/Quick Scan. |
