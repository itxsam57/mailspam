...
3. `EMAIL_SHIELD_RELEASE_SIGNING_KEY_FILE=<owner-only-private.pem> npm run release:sign` signs the already-built portable manifest. `EMAIL_SHIELD_RELEASE_SIGNING_KEY_ID` may pin the expected signer and `EMAIL_SHIELD_SIGNED_UPDATE_OUTPUT` may select an explicit output.
